...
@Rule(
  key = "AV009A",
  name = "Llave { en linea exclusiva",
  description = "Una llave que abre un bloque debe ubicarse en la linea siguiente de manera exclusiva",
  priority = Priority.MINOR,
  tags = {Tag.CONVENTION})
@SqaleSubCharacteristic(RulesDefinition.SubCharacteristics.READABILITY)
@SqaleConstantRemediation("1min")
public class AV009indentacionLlaveAbreJava extends BaseTreeVisitor implements JavaFileScanner {
  ...
  //Implementación de la regla AV009A
  protected  void checkTokens(SyntaxToken lastToken, SyntaxToken openBraceToken){
          if (lastToken.line() == openBraceToken.line()) {
      addIssue(openBraceToken, this, "Una llave que abre un bloque debe localizar en la linea siguiente de manera exclusiva");
    }
  }
